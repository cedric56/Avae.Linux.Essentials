...
namespace Microsoft.Maui.Storage
{
  partial class FilePickerImplementation : IFilePicker
  {
        ...
        {
            var provider = GetStorage();
            if (provider == null) return null;

            var filters = new List<Avalonia.Platform.Storage.FilePickerFileType>();
            if (options?.FileTypes?.Value != null)
            {
                filters.Add(new Avalonia.Platform.Storage.FilePickerFileType("All files")
                {
                    MimeTypes = options.FileTypes.Value.Select(v => MimeHelper.GetMimeType(v)).ToList(),
                    //Patterns = options.FileTypes.Value.ToList()
                });

                filters.AddRange(options.FileTypes.Value.Select(v => new Avalonia.Platform.Storage.FilePickerFileType(v)
                {
    ...
    public partial class FilePickerFileType
    {
        static FilePickerFileType PlatformImageFileType()
          ...
          {
              {  DevicePlatform.Linux, new[] { ".jpg", ".jpeg", ".png", ".gif", ".bmp", ".tiff" } }
          });

        static FilePickerFileType PlatformPngFileType()
          => new FilePickerFileType(new Dictionary<DevicePlatform, IEnumerable<string>>()
          {
              { DevicePlatform.Linux, new[] { ".png" } }
          });

        static FilePickerFileType PlatformJpegFileType()
      => new FilePickerFileType(new Dictionary<DevicePlatform, IEnumerable<string>>()
      {
          { DevicePlatform.Linux, new[] { ".jpg", ".jpeg" } }
      });

        static FilePickerFileType PlatformVideoFileType()
      => new FilePickerFileType(new Dictionary<DevicePlatform, IEnumerable<string>>()
      {
          {      DevicePlatform.Linux, new[] { ".mp4", ".mov", ".avi", ".mkv", ".wmv" } }
      });

        static FilePickerFileType PlatformPdfFileType()
      => new FilePickerFileType(new Dictionary<DevicePlatform, IEnumerable<string>>()
      {
          { DevicePlatform.Linux, new[] { ".pdf" } }
      });
